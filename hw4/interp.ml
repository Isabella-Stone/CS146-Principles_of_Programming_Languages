...
and
  ...
  | SetField(e1,id,e2) ->
   eval_expr e1 >>= fields_of_recordVal >>= fun list ->
   eval_expr e2 >>= fun wanted ->
   let second = List.assoc_opt id list in
   (match second with
   | None -> error "Field not found"
   | Some(true, RefVal a) -> (Store.set_ref g_store a wanted) >>= fun final ->
      return UnitVal 
   | _ -> error "Field not mutable")


  | IsNumber(e) ->
    eval_expr e >>= is_numVal >>= fun num ->
    return (BoolVal num)
